-- Modified: fixing whitespace, merge (most) changes from https://github.com/Phrogz/SLAXML/tree/dev/richdom, added :replace, :clone
...
function SLAXML:remove(node, parent)
    ...
    if parent.el then removeFromArray(parent.el, node) end
    return true
end


local function replaceFromArray(a,v,n)
    for i,v2 in ipairs(a) do if v2==v then a[i] = n return i end end
end

-- Replace a node from the DOM with another; returns false if the node could not be replaced, nil otherwise
function SLAXML:replace(node, newNode)
    if not node.parent then return end
    if node.type~='attribute' and newNode.type=='attribute' then error('Cannot replace non-attribute with attribute') end
    if node.type=='attribute' then
        if newNode.type~='attribute' then error('Cannot replace attribute with non-attribute') end
        node.parent.attr[node.name] = nil
        node.parent.attr[newNode.name] = newNode.value
    end
    replaceFromArray(node.type=='attribute' and node.parent.attr or node.parent.kids, node, newNode)
    if node.parent.el then
        if newNode.type=='element' then
            replaceFromArray(node.parent.el, node, newNode)
        else
            removeFromArray(node.parent.el, node)
        end
    end
...
local function cloneBase(from, newParent)
    return {
        type = from.type,
        name = from.name,
        value = from.value,
        cdata = from.cdata,
        nsURI = from.nsURI,
        nsPrefix = from.nsPrefix,
        parent = newParent,
    }
end

-- Clone (depp copy) node
function SLAXML:clone(node, newParent)
    local newNode = cloneBase(node, newParent)
    if newNode.type == 'element' then
        if node.attr then
            newNode.attr = {}
            for k,v in pairs(node.attr) do
                if type(v) == 'table' then
                    newNode.attr[k] = cloneBase(v, newNode)
                else
                    newNode.attr[k] = v
                end
            end
    ...
    if newNode.type == 'element' or newNode.type == 'document' then
        if node.kids then
            newNode.kids = {}
            if node.el then newNode.el = {} end
            for i,v in ipairs(node.kids) do
                local n = self:clone(v, newNode)
                newNode.kids[i] = n
                if n.type == 'element' then
                    if newNode.type == 'document' then
                        newNode.root = n
                    end
                    if newNode.el then
                        newNode.el[#newNode.el+1] = n
                    end
                end
            end
        end
